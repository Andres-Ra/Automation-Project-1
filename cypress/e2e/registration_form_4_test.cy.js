...
describe('Input fields', () => {
    it('Username cannot be empty string', () => {
        ...
        cy.get('#input_error_message').should('be.visible') //should('be.visible')
    ...
    it('Username tooltip is visible', () => {
        ...
        cy.get('#username').should('have.attr', 'title').should('contain', 'Please add username') //#username not .username
    ...
    it('Username should have min and max length values 1 and 50 characters', () => {
        ...
        cy.get('#username').should('have.attr', 'min', '1') // min '1' not '11'

        // check that username HTML has max attribute value equal to 50
        cy.get('#username').should('have.attr', 'max', '50') //#username not #username 2; '50' not '501'
    })

    it('Username should support only lower letters and numbers', () => {
        // check with regex supporter format
        cy.get('#username').should('have.attr', 'pattern', '[a-zA-Z0-9_]+') //pattern must end with '+' symbol
    })
    ...
    it('Email input should support correct pattern', () => {
        ...
        cy.get('#email').type('invalid') //#email not #email123
        cy.get('h2').contains('Password').click()
        cy.get('#email').should('have.css', 'box-shadow').should('contain', 'rgb(255, 0, 0)') //box-shadow not image
    ...
    it('User cannot submit empty registration form', () => {
        ...
        cy.get('.submit_button').should('not.be.enabled'); //not.be.enabled is correct - not.be.visible is not used for buttons
    })

    it('BMW should not be listed in cars list', () => {
        
        // Check list size is 4
        cy.get('#cars').children().should('have.length', 4) //lenght is 4
        ...
        cy.get('#cars option').last().should('not.have.text', 'BMW') //not.have.text
    })
})